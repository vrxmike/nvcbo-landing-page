...
const FOLDER_PATH = '/home/vamp/Downloads/Wolfram /';

async function uploadAndSeed() {
  const allFiles = fs.readdirSync(FOLDER_PATH).filter(f => !f.startsWith('.') && fs.statSync(path.join(FOLDER_PATH, f)).isFile());
  
  // Filter out duplicates that have "(1)" in the name
  const uniqueFiles = allFiles.filter(f => !f.includes('(1)'));
  
  console.log(`Found ${uniqueFiles.length} unique files in ${FOLDER_PATH}`);

  const MAX_SIZE_BYTES = 50 * 1024 * 1024; // 50MB limit to prevent 503 errors
  ...
  for (let i = 0; i < uniqueFiles.length; i++) {
    ...
    if (stats.size > MAX_SIZE_BYTES) {
        console.log(`[SKIPPED] ${fileName} is ${(stats.size / (1024*1024)).toFixed(2)}MB. Skipping...`);
        skippedCount++;
        continue;
    }
    ...
    try {
      ...
      const rowData = {
        ...
        caption: isVideo 
          ? 'Video footage from the Wolfram STEM programming camp.'
  ...
  console.log(`Upload and seed complete! Successfully processed ${uploadedCount} files into WOLFRAM. Skipped ${skippedCount} large files.`);
}
